...
public class BrokerController {
    ...
    public BrokerController(
        ...
        //broker各种功能对应的组件，例如consumerOffsetManager专门管理Consummer的offset的；
        //topicConfigManager专门管理topic配置的；pullMessageProcessor处理Consummer发送请求过来拉取消息的
        //其他组件也类似，需要在创建brokerController控制器的的时候，创建这些组件。
        ...
        //下面是一大堆线程池的队列：用来实现某些功能的后台线程池的队列。不同的功能用不同的线程池来处理。
        ...
        //broker的功能性组件，例如brokerStatsManager，一看就是metric统计组件，对broker内部的数据进行统计；
        //brokerFastFailure用来处理broker内部故障的组件
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        //加载磁盘上的数据到内存：topic、consumer的offset、consumer的订阅组、过滤器等
        ...
        if (result) {
            try {
                //创建消息存储管理组件，用来管理磁盘上的消息
                this.messageStore =
                    new DefaultMessageStore(this.messageStoreConfig, this.brokerStatsManager, this.messageArrivingListener,
                        this.brokerConfig);
                //若启动了Dleger技术，则初始化相关组件
        ...
        if (result) {
            //初始化Netty，并创建一堆后台线程池，broker有nettyclient和nettyserver端
            ...
            //定时对Consummer filter过滤器进行持久化的任务
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    try {
            ...
            //设置nameServer地址列表，支持不通过配置的方式来写入地址，还可以发送请求来加载
    ...
    public void start() throws Exception {
        ...
        //核心：该组件可以让broker通过Netty客户端发送请求出去给别的服务端，
        // 例如发送请求到nameserver进行注册和心跳连接
        if (this.brokerOuterAPI != null) {
        ...
        //核心：向线程池中提交任务，让它去nameServer进行注册
        //注意：这是个定时任务，第一次是注册，以后每隔30s(brokerConfig.getRegisterNameServerPeriod())发送一次注册请求
        //     这个功能就是每30s发送一次心跳
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
    ...
    public synchronized void registerBrokerAll(final boolean checkOrderConfig, boolean oneway, boolean forceRegister) {
        //暂时不用理会这里面的具体内容，知道是在处理Topic的一些配置就好
        ...
        //关键：判断是否需要注册，需要，则调用doRegisterBrokerAll进行真正的注册
    ...
    private void doRegisterBrokerAll(boolean checkOrderConfig, boolean oneway,
        ...
        //注册的核心代码：通过brokerOuterAPI发送请求给NameServer
        //由于broker需要把自己注册到所有的NameServer中，所以返回结果是个List
