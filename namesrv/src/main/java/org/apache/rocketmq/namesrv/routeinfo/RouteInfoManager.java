...
public class RouteInfoManager {
    ...
    /**
     * 真正的将broker注册到NameServer中，注册的时候虽然数据结构用的hashMap，但注册过程中都加了锁，可以保证安全。
     * @param clusterName
     * @param brokerAddr
     * @param brokerName
     * @param brokerId
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                //在加写锁的状态下，将broker的信息注册到各个数据容器当中
                this.lock.writeLock().lockInterruptibly();
                ...
                //直接将brokerName扔到了这个set集合当中
                //这就是在维护一个集群里有哪些broker存在的一个set数据结构
                //假如后续同一个broker每隔30s发送一次请求，这里的数据是会自动去重的
                brokerNames.add(brokerName);

                boolean registerFirst = false;
                //这是根据brokerName获取到brokerData,使用brokerAddrTable作为核心路由数据表
                //这里存放了所有的broker的详细路由数据
                BrokerData brokerData = this.brokerAddrTable.get(brokerName);
                //如果是第一次发送注册请求，这里就是null
                //那么就会封装一个brokerData，放入到这个路由数据表中
                //这个就是核心的broker注册过程

                //如果后续每个30s发送注册请求作为心跳，这里是不受影响的，因为此时brokerData不为空，
                //brokerAddrTable不会重复处理
                if (null == brokerData) {
                    registerFirst = true;
                ...
                //核心：每隔30s发送注册请求作为心跳的时候，这里才是最核心的处理逻辑：
                //每个30s封装一个新的BrokerLiveInfo放入Map，
                //所以每隔30s，最新的BrokerLiveInfo都会覆盖之前的BrokerLiveInfo
                //这个BrokerLiveInfo中，有一个当前时间戳，代表你最近一次心跳时间
                //这就是broker每隔30s发送一次注册请求作为心跳的处理逻辑
    ...
    public void scanNotActiveBroker() {
        //扫描BrokerLiveInfo这个心跳数据结构
        //遍历，拿到每个Broker最近一次心跳刷新的BrokerLiveInfo
        ///从而知道一个Broker最近一次发送心跳是什么时候
        Iterator<Entry<String, BrokerLiveInfo>> it = this.brokerLiveTable.entrySet().iterator();
        while (it.hasNext()) {
            Entry<String, BrokerLiveInfo> next = it.next();
            long last = next.getValue().getLastUpdateTimestamp();
            //核心判断：
            //如果说当前时间距离上一次心跳时间，超过了broker心跳超时时间(默认为120s，也就是2分钟)
            ///就认为这个broker已经死掉了，并将其从broker心跳路由数据表当中删除，同时摧毁与这个broker建立的Channel
